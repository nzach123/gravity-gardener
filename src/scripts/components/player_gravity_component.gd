...
var baseline_ascent_mag: float = 0.0       # 1.0x reference; zones scale off this
...
func initialize(max_speed: float) -> void:
	...
	baseline_ascent_mag = gravity_ascent_mag
...
## [param direction] need not be normalized; only its angle is used.
## [param multiplier] scales the baseline strength — 1.0 is normal, 0.5 is low
## gravity, 2.0 is heavy.
func set_gravity(direction: Vector2, multiplier: float) -> void:
	if direction.is_zero_approx() or multiplier <= 0.0:
		return
	gravity_ascent_mag  = baseline_ascent_mag * multiplier
	gravity_descent_mag = gravity_ascent_mag / ascent_descent_ratio
	# jump_velocity is deliberately NOT recalculated (GDD R5). Launch speed is
	# fixed, so height varies as v^2 / 2g — heavier zones mean lower jumps, and
	# reachability becomes a level-design lever.
	target_gravity      = direction.normalized() * gravity_ascent_mag
