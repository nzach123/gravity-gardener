...
func _update_visuals(delta: float, right_dir: Vector2, up_dir: Vector2) -> void:
	...
	# Fixed lerp target (was lerping to itself in original)
	sprite.scale = sprite.scale.lerp(sprite.scale, squash_stretch_speed * delta)

	if is_sliding: sprite.play("Slide")
	elif is_crouching: sprite.play("Crouch")
	elif not is_on_floor(): sprite.play("Falling" if velocity.dot(-up_dir) > 0 else "Jump")
