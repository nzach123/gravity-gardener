...
| 3 | **Camera three-way decouple** (`TR-gravity-010`) — `camera_moving` gates rotation and follow together; `camera_rotation_enabled` is separately uncoupled, blocking the reduced-motion accessibility commitment. Camera architecture, not HUD architecture — ADR-0010 explicitly declines it too. | **Unassigned** | Open |
