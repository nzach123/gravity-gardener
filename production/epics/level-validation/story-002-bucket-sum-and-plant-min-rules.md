...
- [ ] `plant.gd` gains `@export_range(1, 4) var buckets_required: int = 1`, matching
...
- [ ] `V-BUCKET-SUM` fires when `count_buckets(level) != sum of every plant's
      buckets_required`, in **both** directions of mismatch.
- [ ] The `V-BUCKET-SUM` finding names both quantities, in the ADR-0003 D3.4 shape:
      `[V-BUCKET-SUM] buckets_total 3 != sum(buckets_required) 4`.
- [ ] `V-BUCKET-SUM` sources its bucket count from `count_buckets()` (story 001) and
      never from an independent count or an authored total.
- [ ] `V-PLANT-MIN` fires once **per offending plant** when that plant's
      `buckets_required < 1`, naming the plant so an author can find it:
      `[V-PLANT-MIN] Plant "Plant2" has buckets_required 0; must be >= 1`.
- [ ] A level breaching both rules returns findings for both in one call — no early
      return (ADR-0003 Validation Criterion 1, scoped to these two rules).
- [ ] A clean level returns empty from `validate()`.
- [ ] `plant.gd`, `level_validation.gd` and the test file are warning-clean under the
